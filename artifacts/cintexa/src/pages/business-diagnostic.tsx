...
import {
  ...
  ChevronRight, CircleHelp, Download, Gauge, GitCompare, Goal, Lightbulb,
  ...
  if (stage === "overview") return (
    <div className="space-y-7 pb-12">
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {[[Gauge, "Adaptive diagnosis", "Questions change as weak signals appear"], [GitCompare, "Benchmarking", "Company, industry and competitor gaps"], [Goal, "Goal cascade", "Strategy → tactics → operations → KPI"], [Bot, "Evidence-aware AI", "Facts and inference remain visibly separate"]].map(([Icon, title, text]) => <Card key={String(title)}><CardContent className="p-5"><div className="p-2 rounded-lg bg-primary/10 w-fit mb-4"><Icon className="w-5 h-5 text-primary" /></div><h3 className="font-semibold">{String(title)}</h3><p className="text-sm text-muted-foreground mt-1">{String(text)}</p></CardContent></Card>)}
      </div>
