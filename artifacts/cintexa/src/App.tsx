...
function lazyWithRecovery<T extends ComponentType<unknown>>(loader: () => Promise<{ default: T }>) {
  return lazy(async () => {
    try {
      ...
      // Vite can leave a browser with a stale HMR/module URL after a file update.
      // Reload once so the browser obtains the current module graph. Do not loop.
...
class RouteErrorBoundary extends Component<{ children: ReactNode }, { error: Error | null }> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error("CINTEXA route error", error, info);
  }
  ...
  render() {
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-8 min-h-[400px]">
        <div className="max-w-lg w-full rounded-xl border bg-card p-6 text-center shadow-sm">
          <h2 className="text-xl font-semibold">This page could not be loaded</h2>
          <p className="text-sm text-muted-foreground mt-2">The application recovered from the page error, but the current module needs to be reloaded.</p>
          {this.state.error?.message && (
            <p className="mt-3 text-xs font-mono text-rose-400/90 break-all max-h-24 overflow-auto">{this.state.error.message}</p>
          )}
          <button className="mt-5 rounded-md bg-primary px-4 py-2 text-primary-foreground text-sm font-medium" onClick={() => window.location.reload()}>Reload CINTEXA Nexus</button>
        </div>
      </div>
    );
  }
